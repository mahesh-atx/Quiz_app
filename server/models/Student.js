...
studentSchema.methods.addQuiz = async function (quizId, teacherId) {
  ...
  const exists = this.quizzesTaken.find(
...
studentSchema.statics.getByTeacher = function (teacherId) {
  ...
  return this.find({ associatedTeachers: teacherId }).sort({ lastActive: -1 });
};
  ...
  results.forEach((result) => {
    if (result.student) {
      ...
      if (
        ...
        result.percentage > studentMap.get(studentId).percentage
      ) {
